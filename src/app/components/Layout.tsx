...
import { Activity, Users, MapPin, Home, LogIn, LogOut, User, Dumbbell, MessageCircle, X, Send, Globe } from "lucide-react";
...
export function Layout() {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (text.includes("chơi ở đâu") || text.includes("tìm sân") || text.includes("cầu lông")) {
        ...
        botReply = "Hệ thống Uy Tín giúp ngăn chặn bùng kèo. Bạn sẽ được +5 điểm mỗi lần tham gia đúng giờ và nhận Huy hiệu khi duy trì Streak 7 ngày đấy! Bạn cũng có thể bị trừ điểm nếu đăng lịch mà không đến.";
  ...
  const navItems = [
    ...
    { path: "/groups", label: "Hội nhóm", icon: Activity },
    ...
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col md:flex-row">
            ...
            <div className="flex flex-col gap-2">
              <Link to="/profile" className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                <div className="w-10 h-10 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold overflow-hidden border border-teal-200">
                  {user?.user_metadata?.avatar_url ? <img src={user?.user_metadata?.avatar_url} className="w-full h-full object-cover" alt={user?.user_metadata?.full_name || user?.email || 'Ảnh đại diện'} /> : <User size={20} />}
                </div>
          ...
          <div className="px-4 py-3 flex justify-between items-center">
            ...
            <div className="flex items-center">
              ...
              <Link to="/profile" className="w-8 h-8 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold overflow-hidden border border-teal-200">
                {user?.user_metadata?.avatar_url ? <img src={user?.user_metadata?.avatar_url} className="w-full h-full object-cover" alt={user?.user_metadata?.full_name || user?.email || 'Ảnh đại diện'} /> : <User size={16} />}
              </Link>

              ) : (
                <Link to="/auth" className="text-teal-600 font-bold text-sm bg-teal-50 px-3 py-1.5 rounded-lg">
                  Đăng nhập
                </Link>
              )}
            </div>
      ...
      {session && !location.pathname.startsWith('/messages') && (
        <div className="fixed bottom-20 md:bottom-6 right-4 md:right-6 z-50 flex flex-col items-end">
          {isChatOpen && (
            <div className="bg-white border border-teal-100 shadow-2xl rounded-2xl w-80 max-h-96 mb-4 flex flex-col overflow-hidden origin-bottom-right">
              <div className="bg-teal-700 text-white p-3 flex justify-between items-center shadow-md">
                ...
                <button onClick={() => setIsChatOpen(false)} className="hover:bg-teal-600 p-1 rounded-md transition-colors" title="Đóng chat"><X size={18} /></button>
              </div>
              <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50 text-sm max-h-64">
                {chatMessages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.isBot ? 'justify-start' : 'justify-end'}`}>
                    <div className={`max-w-[80%] p-2.5 rounded-xl text-slate-800 shadow-sm leading-relaxed ${
                      msg.isBot ? 'bg-white border border-slate-100 rounded-tl-sm' : 'bg-orange-100 border border-orange-200 rounded-tr-sm'
                    }`}>
                      {msg.text}
                    </div>
                  </div>
          ...
          <button 
            ...
            aria-label={isChatOpen ? "Đóng chat" : "Mở chat assistant"}
            title={isChatOpen ? "Đóng chat" : "Mở chat assistant"}
          >
            {isChatOpen ? <X size={24} aria-hidden="true" /> : <MessageCircle size={24} aria-hidden="true" />}
          </button>
